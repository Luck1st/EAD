...
# (Removed duplicate import of ChatPromptTemplate and related classes)
from auxiliary_functions import get_conversation_string, query_refiner, find_match
# from apikey import apikey 
apikey = "sk-proj-KrkWw4v6zmw_HuAzVgjrQkyEwukgHyMNMqsTIFqQSPvdLXFSYOwCjnf4mOLKBAcLQgVB1Wu6-yT3BlbkFJQPdKNfald10QSx1Nzq3N2GH1KhV11OxpNWpDAUyEXuSzI0bCXRrv07ktsz6KM8IjGBfajbYBI"
...
if st.session_state.clicked[1]:
    ...
    with tab2:
        ...
        # Ensure the OpenAI API key is set before initializing ChatOpenAI
        if not os.environ.get("OPENAI_API_KEY") or os.environ.get("OPENAI_API_KEY") in ("", None):
            st.error("OpenAI API key not found or invalid. Please set the OPENAI_API_KEY environment variable.")
            st.stop()
        try:
            # Ensure the API key is set and ChatOpenAI is imported correctly
            if not apikey or apikey in ("", None):
                raise ValueError("OpenAI API key is missing or invalid.")
            # Ensure ChatOpenAI is properly imported and initialized
            try:
                if ChatOpenAI is None or not callable(ChatOpenAI):
                    raise ImportError("ChatOpenAI is not properly imported or is None. Please check your langchain installation.")
                chat_llm = ChatOpenAI(openai_api_key=apikey, model_name="gpt-3.5-turbo")
            except (TypeError, ImportError) as e:
                st.error(f"Failed to initialize ChatOpenAI. Please check your langchain and openai package versions. Error: {e}")
                st.stop()
